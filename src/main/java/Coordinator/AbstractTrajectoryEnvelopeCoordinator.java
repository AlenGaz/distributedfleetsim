...
public abstract class AbstractTrajectoryEnvelopeCoordinator {
	...
	protected static Logger metaCSPLogger = MetaCSPLogging.getLogger(CoordinatorServer.class);
	 ...
	 */
	public boolean isDriving(int robotID) { return (isDriving.containsKey(robotID) && isDriving.get(robotID)); }
	...
	public static String getResourceAsFileName(String resource) {
		...
		ClassLoader classLoader = CoordinatorServer.class.getClassLoader();
...
//						catch (InterruptedException e) { e.printStackTrace(); }
	...
	private static void printLicense() {
		System.out.println("\n"+CoordinatorServer.TITLE);
		System.out.println(CoordinatorServer.COPYRIGHT+"\n");
		if (CoordinatorServer.LICENSE != null) {
			List<String> lic = StringUtils.fitWidth(CoordinatorServer.PRIVATE_LICENSE, 72, 5);
			for (String st : lic) System.out.println(st);
		}
		else {
			List<String> lic = StringUtils.fitWidth(CoordinatorServer.PUBLIC_LICENSE, 72, 5);
			for (String st : lic) System.out.println(st);
		}
		System.out.println();
	}
