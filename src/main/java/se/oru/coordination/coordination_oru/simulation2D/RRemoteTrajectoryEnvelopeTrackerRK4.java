...
public abstract class RRemoteTrajectoryEnvelopeTrackerRK4 extends AbstractTrajectoryEnvelopeTracker implements Runnable {
	...
	public RRemoteTrajectoryEnvelopeTrackerRK4(TrajectoryEnvelope te, int timeStep, double temporalResolution, RemoteTrajectoryEnvelopeCoordinatorSimulation tec, TrackingCallback cb) {
	...
	public RRemoteTrajectoryEnvelopeTrackerRK4(TrajectoryEnvelope te, int timeStep, double temporalResolution, double maxVelocity, double maxAcceleration, TrajectoryEnvelopeCoordinator tec, TrackingCallback cb) {
	...
	protected void onTrajectoryEnvelopeUpdate(TrajectoryEnvelope te) {
		synchronized(reportsList) { //FIX ME not ok, all the mutex should be changed
			this.totalDistance = this.computeDistance(0, traj.getPose().length-1);
			this.overallDistance = totalDistance;
	...
	public void setCriticalPoint(int criticalPointToSet, int extCPCounter) {
		...
		int x = client.makeCriticalPointreq("requestcriticalpoint", te.getRobotID()); //vi får tillbaka VÅR senaste critical point

		System.out.println("IN RK4 x: " + x);
		System.out.println("IN RK4 criticalPointToSet: " + criticalPointToSet);
		System.out.println("Robot ID: " + te.getRobotID());

		//final int criticalPoint = x;
		final int criticalPoint = criticalPointToSet;
		final int externalCPCount = extCPCounter;


		*/
/** network thing, we think its not needed for now
	...
	public void run() {
		...
		while (true) {
			...
			if (state.getPosition() >= this.positionToSlowDown && state.getVelocity() < 0.0) {
				...
				if (!atCP */
/*&& getRobotReport().getPathIndex() == criticalPoint*//*
) {
