...
public abstract class RemoteTrajectoryEnvelopeTrackerRK4 extends RemoteAbstractTrajectoryEnvelopeTracker implements Runnable {
	...
	public static void integrateRK4(State state, double time, double deltaTime, boolean slowDown, double MAX_VELOCITY, double MAX_VELOCITY_DAMPENING_FACTOR, double MAX_ACCELERATION) {
		synchronized(state) {
			...
			//System.out.println("[RemoteTrackerRk4] in rk4 " + dxdt: " + dxdt + " dvdt: " +  dvdt);
 ...
 * **/
	...
	public RobotReport getRobotReport() {
		...
		synchronized(state) {
			...
			//System.out.println("[RemoteTractoryEnvelopeTrackerRk4] poses length: " + traj.getPose().length);
			for (int i = 0; i < poses.length - 1; i++) {
			...
			//System.out.println("[RemoteTracker..RK4] in getRobotReport() pose : " + pose);
...
*/
		...
		* in the Abstract...Coordinator class*/
		...
		while (client.makeRobotReportRequest(myRobotID).getPathIndex() != -1)
